<template>
  <div>
    <div class="mb-4">
      <h1 class="text-xl font-bold text-gray-800">🕰️ 回憶時間軸</h1>
    ...
    <!-- 稀有度圖例 -->
    <div v-if="trips.length" class="flex flex-wrap gap-2 mb-5">
      <span v-for="r in legend" :key="r.key"
            class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium"
            :class="r.pill">
        {{ r.emoji }} {{ r.zh }} {{ counts[r.key] || 0 }}
      </span>
    </div>
    ...
    <p v-else-if="!trips.length" class="text-center text-gray-400 py-12">尚無回憶紀錄</p>

    <div v-else class="space-y-6">
      <section v-for="group in grouped" :key="group.year">
        <h2 class="text-sm font-bold text-gray-400 mb-2 sticky top-0 bg-slate-50/90 backdrop-blur-sm py-1 z-10">
          {{ group.year }}
        </h2>
        <div class="relative pl-6">
          <!-- 縱向時間線 -->
          <div class="absolute left-[7px] top-1 bottom-1 w-px bg-gray-200" />
          <div v-for="t in group.items" :key="t.id" class="relative mb-3">
            <!-- 稀有度圓點 -->
            <span class="absolute -left-[22px] top-4 w-3.5 h-3.5 rounded-full ring-2 ring-white"
                  :class="rarityOf(t.rarity).dot" />
            <div class="rounded-2xl border p-3 flex items-center gap-3"
                 :class="rarityOf(t.rarity).card">
              <div class="flex-1 min-w-0">
                <p class="font-semibold text-sm truncate" :class="rarityOf(t.rarity).name">
                  {{ t.title }}
                </p>
                <p class="text-xs mt-0.5" :class="rarityOf(t.rarity).date">
                  {{ formatRange(t) }}
                </p>
              </div>
              <span class="shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium"
                    :class="rarityOf(t.rarity).pill">
                {{ rarityOf(t.rarity).emoji }} {{ rarityOf(t.rarity).zh }}
              </span>
              <button @click.stop="onQuickShare(t.id)" :disabled="sharingId === t.id"
                      class="shrink-0 p-1.5 -mr-1 rounded-full text-gray-400 hover:text-blue-600
                             hover:bg-white/70 disabled:opacity-40 transition-colors"
                      aria-label="分享這段回憶">
